...
STDS['fashion'] = [0.3530]
MEANS['isic2016'] = [0.4377, 0.4438, 0.4728]
...
class OCTDataset(torch.utils.data.Dataset):
    def __init__(self, path, train=True, transform=None, img_size=(256, 256)):
        ...
        classes = [cls_name for cls_name in os.listdir(self.path) if not cls_name.startswith(".")]
        self.classes = {cls_name: i for i, cls_name in enumerate(classes)}

        self.data = []
        for cls_name in self.classes:
            for name in os.listdir(os.path.join(self.path, cls_name)):
                if not name.startswith("."):
                    self.data.append(os.path.join(self.path, cls_name, name))

        # if train:
        #     self.data = self.data[:(len(self.data) // 10) * 9]
        # else:
        #     self.data = self.data[9*(len(self.data) // 10):]
...
def transform_isic2016(augment=False, from_tensor=False, normalize=True):
    if not augment:
        aug = []
    else:
        aug = [transforms.RandomCrop(150, padding=4)]
        print("Dataset with basic ISIC2016 augmentation")
    ...
    if normalize:
        normal_fn = [transforms.Normalize(mean=MEANS['isic2016'], std=STDS['isic2016'])]
    else:
        normal_fn = []
...
def transform_oct(augment=False, from_tensor=False, normalize=True):
    if not augment:
        aug = []
    else:
        aug = []
...
def load_data(args):
    ...
    if args.dataset.startswith('cifar'):
        ...
        train_dataset = ISIC2016("../",train=True)
        val_dataset = ISIC2016("../",train=False)
        nclass = 2
