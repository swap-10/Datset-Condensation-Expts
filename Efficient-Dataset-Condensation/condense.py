...
def load_resized_data(args):
    ...
    if args.dataset == 'cifar10':
        ...
        train_dataset = ISIC2016(args.data_dir,
                                train=True,
                                transform=transforms.ToTensor())

        normalize = transforms.Normalize(mean=MEANS['isic2016'], std=STDS['isic2016'])
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])

        val_dataset = ISIC2016(args.data_dir, train=False, transform=transform_test)
        ...
        normalize = transforms.Normalize(mean=MEANS['oct'], std=STDS['oct'])
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])
...
def condense(args, logger, device='cuda'):
    ...
    it_log = n_iter // 50
    it_test = [i*10 for i in range(n_iter // 10)]
